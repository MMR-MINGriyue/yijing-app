...
- **纯 App 主线 (iter38, v1.34.0)** — 模拟器实测通过; 动效体系 YiMotion (easeOutExpo/按压0.96/stagger/呼吸动爻/六爻推演动画); SDK 从零搭建, 腾讯 gradle 镜像 + kotlin.incremental=false: yijing-flutter/ 为唯一交付形态 — Flutter MVVM 7 屏全量 + 持久化 + 设置面板数据互通 (PWA 导出格式); 架构见 yijing-flutter/ARCHITECTURE.md
